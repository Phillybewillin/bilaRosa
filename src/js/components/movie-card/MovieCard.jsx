...
const MovieCard = (props) => {
    ...
    return (
        ...
        {isLoading ? (
          ...
          <div className="movie-card" loading="lazy" onClick={() => handlecardClick(item.id, props.category ,item.title || item.name, item.poster_path, )} style={{backgroundImage : `url(${bg})`}}>
            <div className="inCinema">
              <i class='bx bx-badge-check' style={{fontSize :'18px'}}></i>
              {cinemaStatus}
            </div>
          ...
          {saved ? <i class='bx bxs-bookmark-plus'  style={{fontSize :'19px'}}></i> :<i class='bx bx-bookmark-plus' style={{fontSize :'16px'}}></i>}
            ...
            <h4 className='year'>
               {Number.isNaN(year) ? '' : year}
                ...
                <div className="vote">{item.vote_average}</div> </div>
          </div>
           </div>
        </>
    );
